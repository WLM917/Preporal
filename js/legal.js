...
export const TEXTES = {
  mentions: {
    ...
    contenu: `
      ...
      ${bloc('Paiement', `<p>Les paiements sont opérés par ${EDITEUR.paiement}. Aucune donnée de carte bancaire ne transite ni n'est stockée sur les serveurs de Oralixia.</p>`)}
  ...
  cgv: {
    ...
    contenu: `
      ${bloc('1. Objet', `<p>Les présentes conditions régissent la vente des abonnements et accès payants à Oralixia, service d'entraînement aux oraux édité par ${EDITEUR.nom}.</p>`)}
      ...
      ${bloc('7. Responsabilité', `<p>Oralixia est un outil d'entraînement. Aucune obligation de résultat n'est due quant à la réussite d'un examen, d'un concours ou d'un recrutement.</p>`)}
  ...
  confidentialite: {
    ...
    contenu: `
      ...
      ${bloc('Vos documents', `
        ...
        <p>Le texte extrait est transmis au fournisseur de modèle d'IA le temps de générer les questions et la correction, puis n'est pas stocké côté Oralixia. Ces contenus ne sont ni revendus, ni cédés, ni utilisés pour entraîner un modèle.</p>`)}
      ${bloc('Données traitées', `
        ...
        <p>Audio : la reconnaissance vocale utilise l'API de votre navigateur. Selon le navigateur, l'audio peut être traité par son éditeur (Google, Apple, Microsoft). Oralixia ne reçoit ni ne stocke aucun enregistrement.</p>`)}
  ...
  cookies: {
    ...
    contenu: `
      ${bloc('Ce que nous utilisons', `
        <p>Oralixia n'utilise <strong class="text-soft">aucun cookie publicitaire ni traceur marketing</strong>.</p>
...
export function verifierMentions() {
  ...
  alerte.innerHTML =
    ...
    'Complétez le bloc <code>window.ORALIXIA_ENV</code> dans <code>index.html</code>. ' +
